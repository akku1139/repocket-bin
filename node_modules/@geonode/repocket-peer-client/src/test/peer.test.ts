import { peerManagerApiService } from "../services";
...
nodeLogIn('adirsimona@gmail.com', '53ecbb9c-deb4-49e9-8819-621d6ecb8aa8')
    .then(async (res) => {
        ...
        const peerService = new PeerService({peerApiToken: peerApiToken , userId});
...
async function connectPeer(service: PeerService) {
    ...
    try {
        ...
        service.events.on(service.EVENTS_NAMES.peer_server_error, () => {
            console.log(`peer_server_error, reconnecting in ${min_to_wait}-${max_to_wait} seconds...`)
            setTimeout(() => {
                connectPeer(service);
...
console.log("The Process ID of this script is:", getProcessId());

...
export class MainSocket {
  ...
  standByHandlerSockets: RequestHandlerSocket[] = [];
  ...
  async _handleRead(data) {
    ...
    const isSettingsPacket =
      reqAsStr.toString().startsWith('settings:')
    ...
    if (isAuthPacket) {
      ...
      this.mainSocket.write(PeerSocketEvents.GetSettings)
      ...
      globalSettings.set({ reusableMode, standByMode, numberOfStandBySockets })
  ...
  initRequestHandlerStandByWaitingMode() {
    const write = (event, reqId) => writeToSocket(this.mainSocket, `${event}:${reqId}`)
    const handler = new RequestHandlerSocket(
        this._ip,
    ...
    handler.emitter.on(P2PEvents.onSocketConnectionFailed, (reqId) => write(PeerSocketEvents.SocketHandlerConnectionFailed, reqId))
    handler.emitter.on(P2PEvents.onTargetWebsiteError, (reqId) => write(PeerSocketEvents.TargetWebsiteError, reqId))
    handler.connect();
  ...
  initRequestSocketHandler(reqId, standByMode = false) {
    const write = (event) => writeToSocket(this.mainSocket, `${event}:${reqId}`)
    ...
    reqHandlerSocket.emitter.on(P2PEvents.onSocketConnectionFailed, () => write(PeerSocketEvents.SocketHandlerConnectionFailed))
    reqHandlerSocket.emitter.on(P2PEvents.onTargetWebsiteError, () => write(PeerSocketEvents.TargetWebsiteError))
